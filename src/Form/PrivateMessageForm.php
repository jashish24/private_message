...
class PrivateMessageForm extends ContentEntityForm {
  ...
  public function save(array $form, FormStateInterface $formState) {
    ...
    $cuser = $this->userManager->load($this->currentUser->id());
    $data = [
      'private_message' => $this->entity,
      'private_message_thread' => $private_message_thread,
      'user' => $this->userManager->load($this->currentUser->id()),
    ];

    $current_user_last_name = !$cuser->get('field_last_name')->isEmpty() ? $cuser->field_last_name->value : '';
    $current_user_first_name = !$cuser->get('field_first_name')->isEmpty() ? $cuser->field_first_name->value : $cuser->getDisplayName();

    $thread_members_ids = array_diff(array_unique($private_message_thread->getMembersId()), [$this->currentUser->id()]);
    foreach ($thread_members_ids as $key => $thread_member_id) {
      $thread_member = $this->userManager->load($thread_member_id);
      if ($thread_member != null) {
        $firstName = !$thread_member->get('field_first_name')->isEmpty() ? $thread_member->field_first_name->value : $thread_member->getDisplayName();
        $body = htmlspecialchars_decode(\Drupal::token()->replace($firstName . ",<br />You have received the following private message on the NEO Network from ".$current_user_first_name." ".$current_user_last_name.":<br /><br />[private_message:message]<br /><br />You can view the entire thread and reply to this message on the NEO Network website at: <a href='[private_message_thread:url]' target='_blank'>[private_message_thread:url]</a><br /><br />Thank you!", $data));
        $params = [
          'subject' => \Drupal::token()->replace('Private message at [site:name]', $data),
          'body' => $body,
        ];
        if ($thread_member) {
          \Drupal::service('plugin.manager.mail')->mail('neonetwork_utils', 'neonetwork_messages_mail', $thread_member->getEmail(), $langcode, $params, NULL);
        }
      }
